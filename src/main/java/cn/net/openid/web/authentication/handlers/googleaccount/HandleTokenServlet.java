 ...
 * <p>
 * The user will login to the Google account and lend permission for this
 * service to impersonate the user. Upon completion of the login and
 * permission-grant, the user will be redirected to this servlet with the token
 * in the URL.
 * 
 * 
 */
public class HandleTokenServlet extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8706413424566742472L;
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		String sessionToken;
		try {
			sessionToken = AuthSubUtil.exchangeForSessionToken(token, Utility
					.getPrivateKey());
		...
		try {
			...
			Map<String, String> info = AuthSubUtil.getTokenInfo(sessionToken,
					Utility.getPrivateKey());
			for (Iterator<String> iter = info.keySet().iterator(); iter
					.hasNext();) {
